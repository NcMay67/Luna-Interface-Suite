...
-- NC HUB: estilo del botón móvil para reabrir la interfaz.
task.defer(function()
    local MobileSupport = LunaUI:FindFirstChild("MobileSupport")
    if not MobileSupport then
        return
    end

    MobileSupport.AnchorPoint = Vector2.new(0.5, 0)
    MobileSupport.Position = UDim2.new(0.5, 0, 0, 14)
    MobileSupport.Size = UDim2.fromOffset(190, 46)
    MobileSupport.BackgroundColor3 = Color3.fromRGB(23, 18, 42)
    MobileSupport.BackgroundTransparency = 0.08
    MobileSupport.BorderSizePixel = 0

    local Corner = MobileSupport:FindFirstChildOfClass("UICorner")
    if not Corner then
        Corner = Instance.new("UICorner")
        Corner.Parent = MobileSupport
    end
    Corner.CornerRadius = UDim.new(0, 14)

    local Stroke = MobileSupport:FindFirstChildOfClass("UIStroke")
    if not Stroke then
        Stroke = Instance.new("UIStroke")
        Stroke.Parent = MobileSupport
    end
    Stroke.Color = Color3.fromRGB(152, 235, 255)
    Stroke.Transparency = 0.25
    Stroke.Thickness = 1.4

    local Gradient = MobileSupport:FindFirstChild("NC_HUB_Gradient")
    if not Gradient then
        Gradient = Instance.new("UIGradient")
        Gradient.Name = "NC_HUB_Gradient"
        Gradient.Parent = MobileSupport
    end
    Gradient.Rotation = 0
    Gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(38, 30, 51)),
        ColorSequenceKeypoint.new(0.55, Color3.fromRGB(67, 47, 88)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(23, 18, 42))
    })

    for _, Object in ipairs(MobileSupport:GetDescendants()) do
        if Object:IsA("TextLabel") or Object:IsA("TextButton") then
            Object.Text = "ABRIR NC HUB"
            Object.Font = Enum.Font.GothamBold
            Object.TextSize = 14
            Object.TextColor3 = Color3.fromRGB(235, 241, 255)
...
local function Hide(Window, bind, notif)
	...
	if notif then
		Luna:Notification({
    Title = "NC HUB oculto",
    Content = "Toca la barra ABRIR NC HUB de arriba para volver a abrir la interfaz.",
    Icon = "visibility_off",
